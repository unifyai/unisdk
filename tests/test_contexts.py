...
def test_create_context():
    assert len(unisdk.get_contexts()) == 0
    unisdk.create_context("my_context")
    assert len(unisdk.get_contexts()) == 1
    assert "my_context" in unisdk.get_contexts()
...
def test_create_context_forwards_owner_scope(monkeypatch):
    ...
    monkeypatch.setattr(unisdk.contexts.http, "post", _fake_post)
...
def test_create_contexts_names_only():
    assert len(unisdk.get_contexts()) == 0
    unisdk.create_contexts(["foo", "bar"])
    assert len(unisdk.get_contexts()) == 2
    assert "foo" in unisdk.get_contexts()
    assert "bar" in unisdk.get_contexts()


@_handle_project_isolated
def test_create_contexts_dicts():
    assert len(unisdk.get_contexts()) == 0
    unisdk.create_contexts(
        [
    ...
    assert unisdk.get_context("foo")["description"] == "bar"
    assert unisdk.get_context("bar")["description"] == "baz"
...
def test_create_contexts_nested():
    ...
    assert len(unisdk.get_contexts()) == 3
    assert "foo" in unisdk.get_contexts()
    assert "foo/bar" in unisdk.get_contexts()
    assert "foo/bar/baz" in unisdk.get_contexts()
...
def test_get_contexts():
    assert len(unisdk.get_contexts()) == 0
    unisdk.log(x=0, context="a/b")
    unisdk.log(x=1, context="a/b")
    unisdk.log(x=0, context="b/c")
    unisdk.log(x=1, context="b/c")
    contexts = unisdk.get_contexts()
    assert len(contexts) == 2
    assert "a/b" in contexts
    assert "b/c" in contexts
    contexts = unisdk.get_contexts(prefix="a")
    assert len(contexts) == 1
    assert "a/b" in contexts
    assert "a/c" not in contexts
    contexts = unisdk.get_contexts(prefix="b")
    assert len(contexts) == 1
    assert "b/c" in contexts
...
def test_delete_context():
    ...
    assert len(contexts) == 1
    assert "a/b" in contexts
    unisdk.delete_context("a/b")
    assert "a/b" not in unisdk.get_contexts()
    assert len(unisdk.get_logs()) == 0


@_handle_project_isolated
def test_add_logs_to_context():
    l0 = unisdk.log(x=0, context="a/b")
    l1 = unisdk.log(x=1, context="a/b")
    l2 = unisdk.log(x=2, context="b/c")
    l3 = unisdk.log(x=3, context="b/c")
    unisdk.add_logs_to_context(log_ids=[l0.id, l1.id], context="b/c")
    assert len(unisdk.get_logs(context="a/b")) == 2
    assert unisdk.get_logs(context="a/b", return_ids_only=True) == [l1.id, l0.id]
    assert len(unisdk.get_logs(context="b/c")) == 4
    assert unisdk.get_logs(context="b/c", return_ids_only=True) == [
        l3.id,
        l2.id,
        l1.id,
        l0.id,
...
def test_rename_context():
    unisdk.log(x=0, context="a/b")
    unisdk.rename_context("a/b", "a/c")
    contexts = unisdk.get_contexts()
    assert "a/b" not in contexts
    assert "a/c" in contexts
    logs = unisdk.get_logs(context="a/c")
    assert len(logs) == 1
...
def test_get_context():
    ...
    context = unisdk.get_context(name)
...
def test_context_relative():
    unisdk.set_context("A", relative=True)

    unisdk.set_context("B", relative=True)
    current_ctx = unisdk.get_active_context()
    assert current_ctx["read"] == "A/B"
    assert current_ctx["write"] == "A/B"
    assert unisdk.get_context(current_ctx["read"])["name"] == "A/B"

    unisdk.set_context("C", relative=True)
    current_ctx = unisdk.get_active_context()
    assert current_ctx["read"] == "A/B/C"
    assert current_ctx["write"] == "A/B/C"
    assert unisdk.get_context(current_ctx["read"])["name"] == "A/B/C"


@_handle_project_isolated
def test_context_not_relative():
    unisdk.set_context("A", relative=False)
    current_ctx = unisdk.get_active_context()
    assert current_ctx["read"] == "A"
    assert current_ctx["write"] == "A"
    assert unisdk.get_context(current_ctx["read"])["name"] == "A"

    unisdk.set_context("B", relative=False)
    current_ctx = unisdk.get_active_context()
    assert current_ctx["read"] == "B"
    assert current_ctx["write"] == "B"
    assert unisdk.get_context(current_ctx["read"])["name"] == "B"
...
def test_create_context_with_foreign_keys():
    ...
    context = unisdk.get_context("Employees")
...
def test_create_contexts_batch_with_foreign_keys():
    ...
    employee_context = unisdk.get_context("Employees")
...
def test_create_context_without_foreign_keys():
    ...
    context = unisdk.get_context("SimpleContext")
...
def test_foreign_key_cascade_action():
    ...
    dept_log = unisdk.log(dept_name="Engineering", context="Departments")
    ...
    dept_logs = unisdk.get_logs(context="Departments")
    ...
    emp_logs = unisdk.get_logs(context="Employees")
...
def test_nested_contexts_with_foreign_keys():
    ...
    context = unisdk.get_context("org/employees")
...
def test_flat_array_foreign_key():
    ...
    context = unisdk.get_context("Articles")
    assert "foreign_keys" in context
    assert len(context["foreign_keys"]) == 1
...
def test_nested_array_foreign_key():
    ...
    # Verify FK is stored with nested path
    context = unisdk.get_context("Transcripts")
    assert "foreign_keys" in context
    assert len(context["foreign_keys"]) == 1
...
def test_nested_object_foreign_key():
    ...
    # Verify FK is stored with nested path
    context = unisdk.get_context("Records")
    assert "foreign_keys" in context
    assert len(context["foreign_keys"]) == 1
...
def test_mixed_nesting_foreign_key():
    ...
    # Verify FK is stored with nested path
    context = unisdk.get_context("Projects")
    assert "foreign_keys" in context
    assert len(context["foreign_keys"]) == 1
...
def test_flat_array_fk_with_actual_data():
    ...
    # Create some tags
    tag1 = unisdk.log(name="Python", context="Tags")
    tag2 = unisdk.log(name="ML", context="Tags")

    # Get the auto-generated tag_ids from _entries
    tags = unisdk.get_logs(context="Tags")
    ...
    articles = unisdk.get_logs(context="Articles")
...
def test_nested_array_fk_with_actual_data():
    ...
    # Create some images
    img1 = unisdk.log(url="https://example.com/1.jpg", context="Images")
    img2 = unisdk.log(url="https://example.com/2.jpg", context="Images")

    # Get the auto-generated image_ids from _entries
    images = unisdk.get_logs(context="Images")
    ...
    transcripts = unisdk.get_logs(context="Transcripts")
...
def test_delete_context_prefix_collision():
    ...
    assert len(unisdk.get_contexts()) == 5
    ...
    unisdk.delete_context("test_call", delete_children=True)
...
def test_delete_context_without_children():
    ...
    unisdk.delete_context("parent", delete_children=False)
...
def test_delete_context_missing_ok_true():
    ...
    result = unisdk.delete_context("nonexistent_context")
...
def test_delete_context_missing_ok_false():
    ...
    with pytest.raises(RequestError) as exc_info:
        unisdk.delete_context("nonexistent_context", missing_ok=False)
...
def test_delete_context_concurrent_with_missing_ok():
    ...
    unisdk.create_context("concurrent_delete_context")
    assert "concurrent_delete_context" in unisdk.get_contexts()

    num_workers = 10

    def delete_context_task():
        return unisdk.delete_context("concurrent_delete_context")
    ...
    assert "concurrent_delete_context" not in unisdk.get_contexts()
    assert len(unisdk.get_contexts()) == 0
...
def test_create_context_exist_ok_true():
    ...
    result = unisdk.create_context("my_context")
    assert result is None
...
def test_create_context_exist_ok_false():
    ...
    with pytest.raises(RequestError) as exc_info:
        unisdk.create_context("my_context", exist_ok=False)
...
def test_create_context_concurrent_with_exist_ok():
    ...
    def create_context_task():
        return unisdk.create_context("concurrent_context")
    ...
    assert "concurrent_context" in unisdk.get_contexts()
    assert len(unisdk.get_contexts()) == 1
...
def test_create_contexts_exist_ok_true():
    ...
    unisdk.create_contexts(["ctx_a", "ctx_b"])
    assert len(unisdk.get_contexts()) == 2

    # Second call with overlapping contexts should succeed without error
    result = unisdk.create_contexts(["ctx_b", "ctx_c"])
...
def test_create_contexts_exist_ok_false():
    ...
    with pytest.raises(ValueError) as exc_info:
        unisdk.create_contexts(["ctx_b", "ctx_c"], exist_ok=False)
...
def test_create_contexts_concurrent_with_exist_ok():
    ...
    def create_contexts_task():
        return unisdk.create_contexts(contexts_to_create)
